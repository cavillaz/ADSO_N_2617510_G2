...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["registrar"])) {
        ...
        $zona = isset($_POST["tipozona"]) ? $_POST["tipozona"] : "";
        ...
        if (empty($correo) || empty($contrasena) || empty($nombre) || empty($documento) || empty($torre) || empty($apartamento) || empty($celular)) {
            ...
            if ($existeUsuario) {
                ...
                $sql = $conexion->query("INSERT INTO tb_residente(identificacion, nombres_usuario, correo, torre, apartamento, celular, clave) VALUES ('$documento', '$nombre', '$correo', '$torre','$zonas', '$apartamento', '$celular', '$contrasena')");
...
$consultaZona = $conexion->query("SELECT * FROM tb_zonas_comunes");
$zonas = $consultaZona->fetch_all(MYSQLI_ASSOC);
?>